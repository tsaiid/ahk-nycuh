...
/*
^9::
  CopyReportPath := "4.1.4.2.4.1.4.1.4.6.4.6.4"
  ...
  ;btnObj := Acc_Get("Name", CopyReportPath, 0, "報告作業(frmRISReport)")
  ;btnObj := Acc_Get("Object", CopyReportPath, 0, "報告作業(frmRISReport)")
  ;btnObj.accDoDefaultAction(0)
  ;examname := StrReplace(examname, "檢查項目: ", "")
  ;ControlClick WindowsForms10.BUTTON.app.0.2780b98_r24_ad15
  ;MsgBox, %action%
;  Send ^0
;  Sleep, 500
;  Send ^{Home}

  ;ControlGet, hEdit, Hwnd,, WindowsForms10.EDIT.app.0.2780b98_r24_ad11
  ;a := Edit_GetText(hEdit)
  ;MsgBox, %a%

  AccPathToTable := "4.1.4.2.4.1.4.1.4.6.4.1.4"
  oTableAcc := Acc_Get("Object", AccPathToTable, 0, "報告作業(frmRISReport)")
  if !IsObject(oTableAcc)
    {
        MsgBox, 找不到 Table 物件！`n請檢查您的 WinTitle 和 AccPath: %AccPathToTable%`n`n(提示: 請使用 Accessibility Insights 找出正確路徑)
        return
    }
    ...
    Loop, % RowCount
    {
        ...
        ;msgbox % oRowAcc.accName(0)
        ;break
        ;oRowAcc := oTableAcc.accChild(1) ; 獲取「行」物件
        ;CellCount := oRowAcc.accChildCount
        ;msgBox % CellCount
        ;break
        oCell1Acc := oRowAcc.accChild(1)
        oCell3Acc := oRowAcc.accChild(3)
        ;FinalOutput .= RowIndex . ": " . oCell1Acc.accValue(0) . " " . oCell3Acc.accValue(0) . "`n"
        ;FinalOutput .= RowIndex . ": (" . CurrentState . ")" . oRowAcc.accValue(0) . "`n"
        ;break
        ;sleep 100
        ...
        ;if (RowIndex = RandRow) {
          ;Sleep 100
          oCell3Acc.accDoDefaultAction(0)
          Sleep 300
          FinalOutput .= RowIndex . ": " . oCell1Acc.accValue(0) . " " . oCell3Acc.accValue(0) . "`n"
          ;MsgBox, %FinalOutput%

          oCell3Acc.accSelect(3)
          oCell3Acc.accLocation(ByRef x, ByRef y, ByRef w, ByRef h, 0)
          ClickX := x + (w // 2)
          ClickY := y + (h // 2)
          MouseGetPos, OrigX, Orig
          MsgBox, % "Clicking at (" . x . ", " . y . ")"
          ;Click, %ClickX%, %ClickY%
          ;MouseMove, %OrigX%, %OrigY%, 0
          break
        }

            ;oCellAcc := oRowAcc.accChild(1) ; 獲取「儲存格」物件
            ;    CellText := oCellAcc.accName(0) ; 0 = CHILDID_SELF
    }

    ;oRowAcc.accChild(5).accChild(3).accDoDefaultAction(0)
    ;Msgbox %FinalOutput%
    ...
    Loop, % RowCount
    {
        ...
        if !IsObject(oRowAcc) ; 如果該行無法被讀取，跳過
            continue

        ; 6. 取得該行的「欄位數」 (Cell count)
        CellCount := oRowAcc.accChildCount
        RowText := ""
        ...
        {
            CellIndex := A_Index
            oCellAcc := oRowAcc.accChild(CellIndex) ; 獲取「儲存格」物件

            if IsObject(oCellAcc)
            {
                ; 對於表格，.accName(0) 通常是儲存格的文字
                CellText := oCellAcc.accName(0) ; 0 = CHILDID_SELF
                RowText .= CellText . "`t" ; 用 Tab 分隔
            }
            else
            {
                RowText .= "[Err]" . "`t" ; 讀取 Cell 失敗
            }
        }

        RowText := RegExReplace(RowText, "\t$", "") ; 移除該行最後的 Tab
        FinalOutput .= RowText . "`n" ; 換行
    }

    ; 8. 顯示結果
    MsgBox, % "抓取完畢：`n`n" . FinalOutput
Return
